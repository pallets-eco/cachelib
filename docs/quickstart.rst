...
A minimal example of using :class:`~cachelib.simple.SimpleCache` from CacheLib
looks like this:
...
1. Import the :class:`~cachelib.simple.SimpleCache` class from the ``cachelib``
   module.
2. Create a cache instance using ``cache = SimpleCache()``.
3. Set a value in the cache with the key ``'my_key'`` and the value
   ``'my_value'`` using the :meth:`~cachelib.base.BaseCache.set` method.
4. Retrieve the value from the cache using the
   :meth:`~cachelib.base.BaseCache.get` method.
5. Print the retrieved value, which outputs ``my_value``.
6. Delete the value from the cache using the
   :meth:`~cachelib.base.BaseCache.delete` method.
7. Retrieve the deleted value again, which returns ``None`` since it has
   been removed from the cache.

This example uses the :class:`~cachelib.simple.SimpleCache` backend. CacheLib
supports multiple backends, such as :class:`~cachelib.redis.RedisCache`,
:class:`~cachelib.memcached.MemcachedCache`, and more. Each backend is used
